...
class InvoiceGenerator:
    ...
    def _create_custom_styles(self):
        """Create custom paragraph styles — safe against duplicates."""
        defs = [
            ParagraphStyle(
                name='CustomTitle',
        ...
        for sty in defs:
            if sty.name not in self.styles:
                self.styles.add(sty)

    # Alias para compatibilidad con código que llama create_custom_styles()
    ...
    def generate_invoice(self, sale: Sale, filename: str = None) -> str:
        ...
        if not filename:
            output_dir = Path.home() / 'Desktop' / 'La Placita' / 'Tickets'
        ...
        for item in sale.items:
            ...
            agrupado[key]["precio"]    = item.precio_unitario
        ...
        prod_table = Table(table_data, colWidths=[8.5*cm, 1.8*cm, 3.2*cm, 3*cm])
    ...
    def generate_sales_report(self, sales: list, filename: str,
                              ...
                              fecha_hasta: str = None) -> str:
        """Generate sales report PDF"""

        output_dir = Path.home() / 'Desktop' / 'La Placita' / 'Reportes'
        output_dir.mkdir(parents=True, exist_ok=True)
        if not filename:
            filename = str(output_dir / f"reporte_{fecha_desde}_{fecha_hasta}.pdf")
        else:
        ...
        summary = (
            f"<b>Total de ventas:</b> {total_ventas}<br/>"
            f"<b>Ingresos totales:</b> Bs {total_ingresos:.2f}<br/>"
            f"<b>Venta promedio:</b> Bs {promedio:.2f}"
        )
        ...
        if sales:
            ...
            for sale in sales:
                fecha = datetime.fromisoformat(
                    sale.fecha_venta).strftime("%d/%m/%Y")
                table_data.append([
                    sale.numero_factura,
                    fecha,
                    sale.cliente or 'Cliente General',
                    sale.metodo_pago.title(),
                    f"Bs {sale.total:.2f}",
                ])

            table = Table(table_data,
                          colWidths=[1.5*inch, 1*inch, 2*inch, 1*inch, 1*inch])
...
def generate_invoice(sale: Sale, filename: str = None) -> str:
    ...
    return InvoiceGenerator().generate_invoice(sale, filename)
...
def generate_sales_report(sales: list, filename: str,
                          ...
                          fecha_hasta: str = None) -> str:
    """Generate sales report PDF"""
    return InvoiceGenerator().generate_sales_report(
        sales, filename, fecha_desde, fecha_hasta)