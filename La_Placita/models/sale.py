...
class Sale:
    ...
    def create(usuario_id: int, items: List[SaleDetail],
               ...
               descuento: float = 0,
               monto_efectivo: float = 0.0, monto_qr: float = 0.0,
               tipo_pedido: str = "mesa") -> tuple:
        """
        Crea la venta y descuenta stock de productos.
        - Stock nunca queda negativo.
        - Si un producto con control de stock llega a 0, se incluye en alertas.
        - Si el producto tiene 'disponible=0' (vender sin stock), se vende igual.
        Devuelve (sale_id, alertas). sale_id=None si falla.
        """
        alertas = []
        try:
